...
class EDD_Software_Licensing {
	...
	private function is_api_request() {
		...
		$allowed_endpoints = self::allowed_api_endpoints();

		foreach ( $allowed_endpoints as $endpoint ) {

			$trigger = self::is_endpoint_active( $endpoint );
	...
	*/
	...
	function activate_license( $args ) {
		...
		$license_id  = self::get_license_by_key( $key );
		$download_id = self::get_download_by_license( $key );
		...
		$license_expires = self::get_license_expiration( $license_id );
		$license_key     = self::get_license_key( $license_id );
		...
		// no activations left
		if( $result['success'] && ( self::is_at_limit( $license_id, $download_id ) && ! $is_local_url ) && ( self::force_increase() || ! self::is_site_active( $license_id, $url ) ) ) {

			$result['success']   = false;
			$result['error']     = 'no_activations_left';
			$result['max_sites'] = self::get_site_count( $license_id );

		}

		// this license has expired'
		$is_lifetime_license = self::is_lifetime_license( $license_id );
		...
		if( ! empty( $args['item_id'] ) && $result['success'] ) {
			...
			if( ! self::is_download_id_valid_for_license( $args['item_id'], $key ) ) {
				$result['success']   = false;
			...
			if( $result['success'] && ( ! defined( 'EDD_BYPASS_NAME_CHECK' ) || ! EDD_BYPASS_NAME_CHECK ) && ! self::check_item_name( $download_id, $item_name ) ) {
				$result['success']   = false;
		...
		if( $result['success'] ) {
			...
			self::insert_site( $license_id, $url );

			// activate the license
			self::set_license_status( $license_id, 'active' );
			...
			self::log_license_activation( $license_id, $_SERVER );
		...
		$result['expires']       = self::get_license_expiration( $license_id );
	...
	function remote_license_activation( $data ) {
		...
		$result = self::activate_license( $args );
	...
	*/
	...
	function deactivate_license( $args ) {
		...
		$license_expires = self::get_license_expiration( $license_id );
		$license_key     = self::get_license_key( $license_id );
		$license_status  = self::get_license_status( $license_id );
		...
		if( ! empty( $args['item_id'] ) ) {
			...
			// Item names don't match
			if( ( ! defined( 'EDD_BYPASS_NAME_CHECK' ) || ! EDD_BYPASS_NAME_CHECK ) && ! self::check_item_name( $download_id, $item_name ) ) {
		...
		self::delete_site( $license_id, $url );

		if ( ! $is_local_url ) {

			// enter this deactivation in the log
			self::log_license_deactivation( $license_id, $_SERVER );
	...
	function remote_license_deactivation( $data ) {
		...
		$result = self::deactivate_license( $args );
	...
	*/
	...
	function check_license( $args ) {
		...
		$license_id = self::get_license_by_key( $args['key'] );
		...
		if( ! empty( $args['item_id'] ) ) {
			...
			if( ! self::is_download_id_valid_for_license( $args['item_id'], $args['key'] ) ) {
		...
		if( ! $is_local_url && ! self::is_site_active( $license_id, $url ) ) {
	...
	*/
	...
	function process_renewal( $download_id = 0, $payment_id = 0, $type = 'default', $cart_item = array() ) {
		...
		if( is_array( $license_keys ) ) {
			...
			foreach( $license_keys as $key ) {
				...
				$license_id  = self::get_license_by_key( $key );
				$item_id     = self::get_download_id( $license_id );
	...
	function renew_license( $license_id = 0, $payment_id = 0, $download_id = 0 ) {
		...
		$length = self::get_license_length( $license_id, $payment_id, $download_id );

		$expiration = self::get_license_expiration( $license_id );
		...
		self::set_license_expiration( $license_id, $new_expiration );
		self::set_license_status( $license_id, 'active' );
	...
	function get_renewal_url( $license_id = 0 ) {
		...
		$args = array(
			'edd_license_key' => self::get_license_key( $license_id ),
			'download_id'     => self::get_download_id( $license_id )
		);
	...
	*/
	...
	function delete_license( $payment_id, $download_id = 0 ) {
		...
		foreach( $licenses as $license ) {
			...
			$license_key = self::get_license_key( $license->ID );

			if ( ! empty( $download_id ) && ! self::is_download_id_valid_for_license( $download_id, $license_key, true ) ) {
				continue;
			}
	...
	*/
	...
	function get_latest_version( $item_id ) {
		...
		return self::get_download_version( $item_id );
	...
	function get_latest_version_remote( $data ) {
		...
		if ( ! $license ) {
			$response['msg'] = __( 'No license key provided', 'edd_sl' );
		...
		if( empty( $item_id ) ) {
			$item_id = self::get_download_by_license( $license );
		}

		$download    = get_post( $item_id );
		$version     = self::get_latest_version( $item_id );
		...
		$response = array(
			...
			'package'       => self::get_encoded_download_package_url( $item_id, $license, $url ),
			'download_link' => self::get_encoded_download_package_url( $item_id, $license, $url ),
			'sections'      => serialize(
				array(
	...
	*/
	...
	function log_license_activation( $license_id, $server_data ) {
		...
		if( self::force_increase() ) {

			$activation_count = self::get_site_count( $license_id );
			$activation_count++;
	...
	*/
	...
	function is_site_active( $license_id = 0, $site_url = '' ) {
		...
		$site_url = trailingslashit( self::clean_site_url( $site_url ) );

		$sites = array_map( array( self::$instance, 'clean_site_url' ), self::get_sites( $license_id ) );
	...
	*/
	...
	function get_download_by_license( $license_key ) {
		...
		$download_id = false;
		$license_id  = self::get_license_by_key( $license_key );
	...
	function is_download_id_valid_for_license( $download_id = 0, $license_key = '', $bypass_constant = false ) {
		...
		$license_download = (int) self::get_download_by_license( $license_key );
	...
	function get_price_id( $license_id = 0 ) {
		...
		return get_post_meta( $license_id, '_edd_sl_download_price_id', true );
	...
	function get_license_by_purchase( $purchase_id = 0, $download_id = 0, $cart_index = false ) {
	...
	function get_license_status( $license_id ) {
		...
		if ( $license_expires && $license_expires < time() && 'expired' !== $status ) {
			$status = 'expired';
			self::set_license_status( $license_id, $status );
		} elseif( 'expired' === $status && $license_expires > time() ) {
			$status = self::get_site_count( $license_id ) >= 1 ? 'active' : 'inactive';
			self::set_license_status( $license_id, $status );
		}
	...
	function get_license_length( $license_id = 0, $payment_id = 0, $download_id = 0 ) {
		...
		$license_length = apply_filters( 'edd_sl_license_exp_lengh', $expiration, $payment_id, $download_id, $license_id ); // for backward compatibility
		$license_length = apply_filters( 'edd_sl_license_exp_length', $expiration, $payment_id, $download_id, $license_id );
	...
	function set_license_expiration( $license_id, $expiration ) {
		...
		foreach ( $child_licenses as $child_id ) {
			self::set_license_expiration( $child_id, $expiration );
		}
	}
	...
	function set_license_as_lifetime( $license_id ) {
		...
		foreach ( $child_licenses as $child_id ) {
			self::set_license_as_lifetime( $child_id );
		}
	...
	function get_license_limit( $download_id = 0, $license_id = 0 ) {
		...
		if ( $license && $license->post_parent && get_post_status( $license->post_parent ) ) {
			...
			return self::get_license_limit( $download_id, $license_id );
		}
		...
		if( '' != $l_limit ) {
			...
			if( edd_has_variable_prices( $download_id ) ) {
				...
				$price_id = (int) self::get_price_id( $license_id );
				if( false !== $price_id ) {
					$price_limit = self::get_price_activation_limit( $download_id, $price_id );
	...
	function license_limit( $license_id = 0 ) {
		...
		$download_id = self::get_download_id( $license_id );
		$limit       = self::get_license_limit( $download_id, $license_id );
	...
	function is_renewal( $payment_id = 0 ) {
		$renewal = get_post_meta( $payment_id, '_edd_sl_is_renewal', true );
		$ret     = false;
